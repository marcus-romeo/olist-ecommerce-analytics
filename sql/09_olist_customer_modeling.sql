...
-- WORKFLOW STAGE: 9 of 15; creates a broad analytical table before the leakage-safe Model A views.
...
-- This intermediate table is intentionally broader than Model A. It supports descriptive analysis,
-- while scripts 11 and 14 define the prediction-time-safe feature tables used for modeling.
...
SELECT
    ...
    -- The following fields occur after purchase and are descriptive only; they are never Model A predictors.
    f.review_score,
    f.delivery_days,
    f.delivery_variance_days,
    f.delivery_status,
    -- Freight burden is known from the initial event and is safe for Model A when the event total is positive.
    CASE
...
JOIN customer_first_purchase_90d f
    -- Both sources are already customer_unique_id grain, so this join should remain one-to-one.
